...
void Game::execute_option_two(){ 
    if((difficulty!="HumanVsHuman")&&(difficulty!="ComputerVsComputer")){
        ...
        cout << "Choose who starts the game "<<"Enter a number. " <<endl;
        cout << "1 - First Human  ";
        cout << "2 - 2nd Human "<<endl;
        cin >> starter;
        if((starter==1) || (starter==2)){
            difficulty==""?difficulty="Elementary":difficulty;
            gameMenu();
...
void  Game::checkAndComputeStats3(){
    if((checkTable() == 'X') && (computer == 'X')){
        ...
        clearArray();
        playGame();
    }else if((checkTable() == 'O') && (computer2 == 'O')){
